...
def _safe_divide(numerator: float, denominator: float) -> float | None:
    """Safely divide and return None for zero or non-finite denominators."""
...
def compute_weekly_nutrition_metrics(nutrition_df: pd.DataFrame, calorie_target: float | None = None) -> dict[str, Any]:
    ...
    weekly["protein_per_lb"] = [
        _safe_divide(float(row["avg_protein_g"]), float(row["avg_bodyweight_lb"]))
    ...
    return {
        "weekly": [
            {
                ...
                "protein_per_lb": (None if pd.isna(row["protein_per_lb"]) else float(row["protein_per_lb"])),
        ...
        "latest": (
            ...
            if latest is None
                ...
                "protein_per_lb": (None if pd.isna(latest["protein_per_lb"]) else float(latest["protein_per_lb"])),
                "calorie_adherence_pct": (
...
def compute_weekly_training_metrics(training_df: pd.DataFrame) -> dict[str, Any]:
    ...
    weekly_top_set["strength_velocity_pct"] = [
        (
            None
            if pd.isna(row["prior_week_top_set"])
            else (
                None
                if _safe_divide(
                    float(row["current_week_top_set"]) - float(row["prior_week_top_set"]),
                    float(row["prior_week_top_set"]),
                )
                is None
                else _safe_divide(
                    float(row["current_week_top_set"]) - float(row["prior_week_top_set"]),
...
def detect_risk_flags(nutrition_metrics: dict[str, Any], training_metrics: dict[str, Any]) -> list[str]:
    ...
    if not volume_df.empty:
        ...
        volume_df["volume_change_pct"] = [
            (
                None
                if pd.isna(row["prior_volume"])
                else (
                    None
                    if _safe_divide(float(row["total_volume"]) - float(row["prior_volume"]), float(row["prior_volume"]))
                    is None
                    else _safe_divide(float(row["total_volume"]) - float(row["prior_volume"]), float(row["prior_volume"]))
                    * 100.0
                )
            )
